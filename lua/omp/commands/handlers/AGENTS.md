...
- Prefer `services/*` over direct new `omp.session` / `omp.api` requires in handlers
...
The following handler files still directly require `omp.session` and should be routed via services APIs.

- [ ] `lua/omp/commands/handlers/diff.lua` -> `omp.session`
- [ ] `lua/omp/commands/handlers/session.lua` -> `omp.session`

Sync rule:
- keep this list aligned with `lua/omp/services/AGENTS.md`
...
- Any new direct `require('omp.session')` or `require('omp.api')` in handlers without exception note
