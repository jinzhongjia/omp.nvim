...
local quick_chat = require('omp.quick_chat')
...
local function run_with_opts(prompt, opts)
  return session_runtime.open(opts):and_then(function()
    return require('omp.services.messaging').send_message(prompt, opts)
  end)
end
...
local function run_debug_action(debug_action)
  ...
  local debug_helper = require('omp.ui.debug_helper')
...
function M.actions.select_history()
  require('omp.ui.history_picker').pick()
end
...
function M.actions.references()
  require('omp.ui.reference_picker').pick()
end
...
function M.actions.mention_file()
  local picker = require('omp.ui.file_picker')
...
function M.actions.next_message()
  require('omp.ui.navigation').goto_next_message()
end

function M.actions.prev_message()
  require('omp.ui.navigation').goto_prev_message()
end

function M.actions.next_user_message()
  require('omp.ui.navigation').goto_next_user_message()
end

function M.actions.prev_user_message()
  require('omp.ui.navigation').goto_prev_user_message()
end

function M.actions.clear_selections()
  require('omp.context').clear_selections()
  vim.notify('Selections cleared', vim.log.levels.INFO)
end

function M.actions.clear_files()
  require('omp.context').clear_files()
  vim.notify('Mentioned files cleared', vim.log.levels.INFO)
end

function M.actions.jump_to_file()
  require('omp.ui.navigation').jump_to_file_at_cursor()
end

function M.actions.jump_to_target_at_cursor()
  require('omp.ui.navigation').jump_to_target_at_cursor()
end
...
function M.actions.navigate_to_location(file_path, line, col)
  require('omp.ui.navigation').navigate_to_location(file_path, line, col)
end
...
M.actions.review = Promise.async(function(args)
  ...
  if not require('omp.services.agent_model').initialize_current_model():await() or not state.current_model then
