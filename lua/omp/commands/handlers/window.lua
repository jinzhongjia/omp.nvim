...
local function build_toggle_open_context(restore_hidden)
  ...
  local focus = config.ui.input.auto_hide and 'input' or state.last_focused_omp_window or 'input'
...
function M.actions.toggle_focus(new_session)
  if not ui.is_omp_focused() then
    local focus = state.last_focused_omp_window or 'input' ---@cast focus 'input' | 'output'
...
M.command_defs = {
  open = {
    desc = 'Open omp window (input/output)',
  ...
  close = {
    desc = 'Close omp windows',
    execute = M.actions.close,
  },
  hide = {
    desc = 'Hide omp windows (preserve buffers for fast restore)',
  ...
  toggle = {
    desc = 'Toggle omp windows',
    execute = M.actions.toggle,
  },
  toggle_focus = {
    desc = 'Toggle focus between omp and code',
