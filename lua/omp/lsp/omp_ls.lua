---In-process LSP server for omp completion
...
handlers[ms.workspace_executeCommand] = function(params, callback)
  if params.command == 'omp.completion_done' then
    ...
    if item then
      require('omp.ui.completion').on_completion_done(item)
    end
    callback(nil, nil)
  else
    callback({
...
function M.supports_kind_icons()
  return require('omp.ui.completion').supports_kind_icons()
end

---Convert omp CompletionItem to LSP CompletionItem
...
local function to_lsp_item(item, index, params, word)
  local source = require('omp.ui.completion').get_source_by_name(item.source_name)
...
function M.start(bufnr)
  ...
  local augroup = vim.api.nvim_create_augroup('OmpLspCompletion_' .. bufnr, { clear = true })
  ...
  vim.api.nvim_create_autocmd('CompleteDonePre', {
    ...
    callback = function()
      ...
      if completed_item and completed_item.user_data then
        ...
        local item = data and data._omp_item
