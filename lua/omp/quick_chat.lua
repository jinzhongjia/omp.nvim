...
---@type table<string, OmpQuickChatRunningSession>
...
---@param message OmpMessage Message object
...
---@param session_info table Session tracking info
---@param messages OmpMessage[] Session messages
...
local function process_response(session_info, messages, range)
  ...
  ---@cast response_message OmpMessage
...
local on_done = Promise.async(function(active_session)
  ...
  local messages = session.get_messages(active_session):await() --[[@as OmpMessage[] ]]
...
---@return OmpContextConfig context_opts
...
---@param context_config OmpContextConfig Context configuration
...
function M.setup()
  local augroup = vim.api.nvim_create_augroup('OMPQuickChat', { clear = true })
