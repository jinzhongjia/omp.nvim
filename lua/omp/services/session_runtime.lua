...
local rpc_manager = require('omp.rpc_manager')
...
M.check_cwd = function()
  if state.current_cwd ~= vim.fn.getcwd() then
    ...
    if state.api_client and state.api_client.set_cwd then
      state.api_client:set_cwd(vim.fn.getcwd())
    end
...
M.open = Promise.async(function(opts)
  ...
  local server = rpc_manager.ensure_started():await()
...
M.cancel = Promise.async(function()
  if state.active_session then
    if state.jobs.is_running() then
      vim.g.omp_abort_count = (vim.g.omp_abort_count or 0) + 1
    end
    ...
    if (vim.g.omp_abort_count or 0) >= 3 then
  ...
  if state.ui.is_visible() then
    require('omp.ui.footer').clear()
    input_window.set_content('')
    require('omp.history').index = nil
    ui.focus_input()
  end
end)

M.omp_ok = Promise.async(function()
  if vim.fn.executable(config.omp_executable) == 0 then
    vim.notify(
      'omp command not found - please install and configure omp before using this plugin',
      vim.log.levels.ERROR
    )
    return false
  end

  if not state.omp_cli_version then
    local promise = Promise.system({ config.omp_executable, '--version' }):and_then(function(result)
    ...
    state.jobs.set_omp_cli_version(promise)
  end

  local required = state.required_version
  local current_version = state.omp_cli_version:await()

  if not current_version or current_version == '' then
    vim.notify(string.format('Unable to detect omp CLI version. Requires >= %s', required), vim.log.levels.ERROR)
    return false
  end

  if not util.is_version_greater_or_equal(current_version, required) then
    vim.notify(
      string.format('Unsupported omp CLI version: %s. Requires >= %s', current_version, required),
      vim.log.levels.ERROR
    )
    return false
  end
...
M._on_user_message_count_change = Promise.async(function(_, new, old)
  require('omp.ui.renderer.flush').flush_pending_on_data_rendered()
