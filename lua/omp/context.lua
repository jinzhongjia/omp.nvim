...
local QuickChatContext = require('omp.context.quick_chat_context')
local BaseContext = require('omp.context.base_context')

local M = {}

---@type table<OmpToggleableContextKey, boolean>
...
---@param context_key OmpToggleableContextKey
---@return table
...
---@param context_config? OmpContextConfig Optional context config
---@param opts? { range?: { start: integer, stop: integer } }
---@return table result { parts: OmpMessagePart[] }
...
---@param context_config? OmpContextConfig Optional context config
---@param opts? { range?: { start: integer, stop: integer } }
---@return table result { text: string, parts: OmpMessagePart[] }
...
--- into the omp input buffer, in the form:
---
---   **`path/to/file`**
---
---   ```<filetype>
...
---@param prompt string
---@param opts? OmpContextConfig|nil
...
--- Extracts context from an OmpMessage (with parts)
---@param message { parts: OmpMessagePart[] }
---@return { prompt: string|nil, selected_text: string|nil, current_file: string|nil, mentioned_files: string[]|nil}
function M.extract_from_omp_message(message)
...
function M.setup()
  ...
  state.store.subscribe({ 'current_code_buf', 'current_context_config', 'is_omp_focused' }, function()
    debounced_load()
  end)

  local augroup = vim.api.nvim_create_augroup('OMPContext', { clear = true })
