...
---@class OmpUiStateMutations
local M = {}

local _state = store.state()

---@param windows OmpWindowState|nil
...
function M.set_panel_focused(is_focused)
  return store.set('is_omp_focused', is_focused)
end

---@param win_type 'input'|'output'|nil
function M.set_last_focused_window(win_type)
  return store.set('last_focused_omp_window', win_type)
end
...
function M.set_last_code_window(win_id)
  return store.set('last_code_win_before_omp', win_id)
end
...
function M.set_code_context(win_id, bufnr)
  store.batch(function()
    store.set('last_code_win_before_omp', win_id)
...
---@param hidden OmpHiddenBuffers|nil
---@return OmpHiddenBuffers|nil
...
---@return {status: 'closed'|'hidden'|'visible', position: string, windows: OmpWindowState|nil, cursor_positions: {input: integer[]|nil, output: integer[]|nil}}
