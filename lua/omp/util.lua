...
function M.safe_pcall(fn, ...)
  ...
  if not ok then
    vim.schedule(function()
      vim.notify('[omp.nvim] Unexpected error: ' .. vim.inspect(err))
    end)
  end
end
...
function M.parse_run_args(args)
  ...
  if prompt:find('#') then
    ...
    opts.context = vim.tbl_deep_extend('force', opts.context or {}, quick_context) --[[@as OmpContextConfig]]
  end
...
--- @return string prompt, OmpContextConfig config
function M.parse_quick_context_args(prompt)
  ---@type OmpContextConfig
