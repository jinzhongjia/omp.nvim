...
local M = {}
local HIDDEN_MESSAGES_NOTICE_MESSAGE_ID = '__omp_hidden_messages_notice__'
local HIDDEN_MESSAGES_NOTICE_PART_ID = '__omp_hidden_messages_notice_part__'
...
function M.setup_subscriptions(subscribe)
  ...
  if subscribe then
    state.store.subscribe('is_omp_focused', M.on_focus_changed)
    state.store.subscribe('active_session', M.on_session_changed)
  else
    state.store.unsubscribe('is_omp_focused', M.on_focus_changed)
...
function M.render_from_cache(session_data)
  ...
  if active_session and active_session.id then
    require('omp.ui.question_window').restore_pending_question(active_session.id)
...
function M.render_lines(lines)
  local output = require('omp.ui.output'):new()
  output.lines = lines
