...
local function scroll(force)
  require('omp.ui.renderer').scroll_to_bottom(force)
end
...
function M.render_permissions_display()
  ...
  M.on_message_updated(fake_message --[[@as OmpMessage]])
...
function M.on_message_updated(message, revert_index)
  ...
  local msg = message --[[@as OmpMessage]]
  ...
  if not revert_index and not ctx.bulk_mode and msg.info.id ~= '__omp_hidden_messages_notice__' then
    require('omp.ui.renderer').reconcile_rendered_message_limit()
  end
end
...
function M.on_message_removed(properties)
  ...
  if not ctx.bulk_mode and message_id ~= '__omp_hidden_messages_notice__' then
    require('omp.ui.renderer').reconcile_rendered_message_limit()
  end
end

---Handle message.part.updated — insert or replace a part in the buffer
---@param properties {part: OmpMessagePart}
---@param revert_index? integer
...
function M.on_session_updated(properties)
  ...
  if revert_changed then
    local real_messages = vim.tbl_filter(function(msg)
      return not (msg.info and msg.info.id and msg.info.id:match('^__omp_'))
    end, state.messages or {})
    require('omp.ui.renderer')._render_full_session_data(real_messages)
  end
end
...
function M.on_session_compacted(properties)
  ...
  require('omp.ui.renderer').render_full_session()
end
...
---@param permission OmpPermission
...
---@param properties OmpQuestionRequest
