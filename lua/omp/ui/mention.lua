...
local mentions_namespace = vim.api.nvim_create_namespace('OmpMentions')
...
---@param mentions OmpMessagePartSourceText[] Mention data with character offsets
  ...
  for _, mention in ipairs(mentions) do
    ...
    for i, line in ipairs(vim.split(text, '\n')) do
      ...
      if char_start == 0 and string.sub(text, 0, 1) ~= '@' then
        -- Work around Omp bug? where mentions sometimes have a 0 start
...
function M.restore_mentions(buf)
  ...
  M.highlight_all_mentions(buf, function(name)
    local agents = require('omp.config_file').get_subagents()

    if vim.tbl_contains(agents, name) then
      require('omp.context').add_subagent(name)
      return
    end

    if vim.fn.filereadable(name) == 1 then
      require('omp.context').add_file(name)
      return
    end
    if name:find('^pasted_image_') then
      local restored_path = require('omp.image_handler').restore_img_path(name)
      if restored_path then
        require('omp.context').add_file(restored_path)
      end
    end
  end)
end
