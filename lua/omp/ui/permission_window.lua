...
function M.remove_permission(permission_id)
  ...
  require('omp.ui.renderer.events').render_permissions_display()
end

---Get currently selected permission (always the first one now)
---@return OmpPermission|nil
...
function M.format_display(output)
  ...
  M._dialog:format_dialog(output, {
    ...
    title_hl = 'OmpPermissionTitle',
    border_hl = 'OmpPermissionBorder',
    content = content,
...
function M._setup_dialog()
  ...
  local function check_focused()
    ...
    return ui.is_omp_focused() and #M._permission_queue > 0
  end
  ...
  local function on_select(index)
    ...
    vim.schedule(function()
      ...
      if action == 'deny' then
        ...
        local part_data = require('omp.ui.renderer.ctx').render_state:get_part('permission-display-part')
        ...
        if output_win and vim.api.nvim_win_is_valid(output_win) then
          ...
          interaction.feedback = require('omp.ui.inline_input').open({
            win = output_win,
            row = row,
            col = col,
            title = 'Tell OMP what to do differently',
  ...
  M._dialog = Dialog.new({
    ...
    on_dismiss = function()
      ...
      if interaction.deny_armed then
        clear_deny_timer(interaction)
        M._processing = true
        require('omp.api').permission_deny(current_permission, nil)
    ...
    namespace_prefix = 'omp_permission',
