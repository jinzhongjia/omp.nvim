...
M._animation = {
  ...
  fps = 10,
  extmark_id = nil,
  ns_id = vim.api.nvim_create_namespace('omp_loading_animation'),
...
M.render = vim.schedule_wrap(function(windows)
  ...
  M._animation.extmark_id = vim.api.nvim_buf_set_extmark(windows.footer_buf, M._animation.ns_id, 0, 0, {
    ...
    virt_text = { { loading_text, 'OmpHint' } },
...
function M.sync_from_server()
  ...
  api_client
    ...
    :catch(function(err)
      require('omp.log').debug('loading_animation.sync_from_server failed: %s', tostring(err))
    end)
end
