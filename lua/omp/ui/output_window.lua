...
local M = {}
M.namespace = vim.api.nvim_create_namespace('omp_output')
M.debug_namespace = vim.api.nvim_create_namespace('omp_output_debug')
M.markdown_namespace = vim.api.nvim_create_namespace('omp_output_markdown')
...
function M.setup(windows)
  ...
  window_options.set_window_option('foldtext', 'v:lua.omp_fold_text()', windows.output_win)
...
function M.setup_autocmds(windows, group)
  ...
  vim.api.nvim_create_autocmd('TabEnter', {
    ...
    callback = function()
      if state.ui.is_window_in_current_tab(windows.output_win) then
        require('omp.ui.renderer.flush').resume_deferred_rendering()
      end
    end,
  })
  ...
  debounced_load_more_at_top = require('omp.util').debounce(function()
    local renderer = require('omp.ui.renderer')
    ...
    if top_line then
      for _, msg in ipairs(state.messages or {}) do
        ...
        if not msg_id:match('^__omp_') then
