...
local session_scope = require('omp.ui.session_scope')
...
---@param parts OmpMessagePart[]|nil
---@param question_request OmpQuestionRequest|nil
---@return OmpMessagePart|nil
...
local function render_question()
  require('omp.ui.renderer.events').render_question_display()
end

---@param question_request OmpQuestionRequest
...
---@param question_info OmpQuestionInfo
---@return integer|nil
...
local function open_inline_other_input(request_id, question_index, option_index, on_submit)
  ...
  local part_data = require('omp.ui.renderer.ctx').render_state:get_part('question-display-part')
  ...
  M._clear_inline_input()
  local handle
  handle = require('omp.ui.inline_input').open({
...
function M.format_display(output)
  ...
  M._dialog:format_dialog(output, {
    ...
    title_hl = 'OmpQuestionTitle',
    border_hl = 'OmpQuestionBorder',
    content = vim.split(question_info.question, '\n'),
    options = options,
    unfocused_message = 'Focus Omp window to answer question',
  })
end
...
function M._setup_dialog()
  ...
  local function check_focused()
    ...
    return ui.is_omp_focused() and not M._answering and is_active_question(request_id, question_index)
  end
