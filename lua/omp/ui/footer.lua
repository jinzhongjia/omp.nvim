...
local function build_right_segments()
  ...
  if loading_animation.is_running() then
    ...
    table.insert(segments, { string.format('%s ', cancel_keymap), 'OmpInputLegend' })
    table.insert(segments, { 'to cancel', 'OmpHint' })
...
function M.render()
  ...
  ---@cast state.windows OmpWindowState
...
function M.setup(windows)
  ...
  vim.api.nvim_set_option_value('winhl', 'Normal:OmpHint', { win = windows.footer_win })
  ...
  vim.api.nvim_create_autocmd({ 'VimResized', 'WinResized' }, {
    group = vim.api.nvim_create_augroup('OmpFooterResize', { clear = true }),
    callback = function()
...
function M.create_buf()
  ...
  vim.api.nvim_set_option_value('filetype', 'omp_footer', { buf = footer_buf })
  return footer_buf
end
...
function M.set_content(lines, highlights)
  ...
  local ns_id = vim.api.nvim_create_namespace('omp_footer')
