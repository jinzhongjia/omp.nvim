...
local footer = require('omp.ui.footer')
...
---@param windows OmpWindowState
---@return OmpHiddenBuffers
local function capture_hidden_snapshot(windows)
  local current_win = vim.api.nvim_get_current_win()
  local focused = state.last_focused_omp_window or 'input'
...
---Clear Omp-specific autocmds and shared UI state before closing windows.
local function prepare_window_close()
  if M.is_omp_focused() then
  ...
  pcall(vim.api.nvim_del_augroup_by_name, 'OmpResize')
  pcall(vim.api.nvim_del_augroup_by_name, 'OmpWindows')
  pcall(vim.api.nvim_del_augroup_by_name, 'OmpFooterResize')
...
function M.restore_hidden_windows()
  ...
  require('omp.ui.contextual_actions').setup_contextual_actions(windows)
...
---Return focus to the window that was active before opening Omp.
function M.return_to_last_code_win()
  local last_win = state.last_code_win_before_omp
...
function M.create_windows()
  if config.ui.enable_treesitter_markdown then
    ...
    if ok and treesitter and treesitter.language and treesitter.language.register then
      treesitter.language.register('markdown', 'omp_output')
      treesitter.language.register('markdown', 'omp')
    end
  end
...
---@return boolean
function M.is_omp_focused()
  if not state.windows then
    return false
  end
  local current_win = vim.api.nvim_get_current_win()
  return M.is_omp_window(current_win)
end

---@param win integer
---@return boolean
function M.is_omp_window(win)
  local windows = state.windows
  if not windows then
    return false
  end
...
---@param opts? {force_scroll?: boolean}
---@return Promise<OmpMessage[]> | OmpMessage[] | nil
