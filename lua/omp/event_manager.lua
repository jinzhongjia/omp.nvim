...
--- @field properties {part: OmpMessagePart}
...
--- @field properties OmpPermission
...
--- @field properties OmpQuestionRequest
...
--- @field state_server_listener function|nil Listener for state.rpc_manager updates
...
--- @param event_name OmpEventName The event name to listen for
...
--- @overload fun(self: EventManager, event_name: "custom.emit_events.started", callback: fun(): nil)
--- @overload fun(self: EventManager, event_name: "custom.emit_events.finished", callback: fun(): nil)
--- @param event_name OmpEventName The event name
...
function EventManager:start()
  ...
  if self.state_server_listener then
    state.store.unsubscribe('rpc_manager', self.state_server_listener)
  end
  ...
  state.store.subscribe('rpc_manager', self.state_server_listener)
  ...
  self.state_cwd_listener = function(key, new_cwd, old_cwd)
    if new_cwd ~= old_cwd and state.rpc_manager and state.rpc_manager.url then
      log.debug('Directory changed from %s to %s, re-subscribing to server events', old_cwd, new_cwd)
      self:_subscribe_to_server_events(state.rpc_manager)
    end
  end
